...
CONTAINS 
   ...
   SUBROUTINE KEB_apply( kt )
      ...
      IF (KEB_negvisc) CALL KEB_negvisc_tendency(  )
      IF (KEB_AR1    ) CALL KEB_AR1_tendency(  )
      
      CALL KEB_statistics( )
      
   END SUBROUTINE KEB_apply
   
   SUBROUTINE KEB_negvisc_tendency(  )      
      !!----------------------------------------------------------------------
      !!                  ***  ROUTINE KEB_negvisc_tendency  ***   
      !! ** Purpose :   Computes negative viscosity, and momentum tendency
      !!                Computes Eback, updates TKE
      !!----------------------------------------------------------------------
      INTEGER  :: jk
      
      ! compute negative viscosity at now time step
      DO jk = 1, jpkm1
         nu2t(:,:,jk) = sqrt(2._wp) * c_back * sqrt(e12t(:,:) * max(TKE(:,:,jk),0._wp)) ! lbc_lnk is already applied to TKE
      END DO

      CALL KEB_ldf_lap( rotb, nu2t, nback, Eback, rhsu, rhsv, ffmask ) 

      ua = ua + rhsu
      va = va + rhsv

      CALL iom_put('negviscx', rhsu)
      CALL iom_put('negviscy', rhsv)
   ...
   SUBROUTINE KEB_AR1_tendency( )      
      ...
      CALL compute_Eback_AR1( fx, fy, un, vn, Eback )
      ...
      ! modified LF scheme (see NEMO book)
      ua = ua + (fx + fxb) * 0.5_wp
      va = va + (fy + fyb) * 0.5_wp   
